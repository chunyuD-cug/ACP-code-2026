...
% Doug Drob. They also wrote an NRLMSISE-00 distribution package in
...
%
% Last modified:   2022/06/24   Meysam Mahooti
%
...
%
% -------------------------------------------------------------------- %
function output = nrlmsise00_own(year,doy,sec,alt,lat,lon,lst,f107,ap)
...
nrl_coeff
gsurf = 0;
re = 6367.088132098377173; % Earth's radius [km]
dd = 0;
dm04 = 0;
dm16 = 0;
dm28 = 0;
...
input = struct('year',1,'doy',1,'sec',1,'alt',1,'g_lat',1,'g_long',1,'lst',1,'f107A',1,'f107',1,'ap',1,'ap_a',ap_a);
...
for i=1:17
    input(i).doy=doy;
    input(i).year=year; % without effect
    input(i).sec=sec;
    input(i).alt=alt;
    input(i).g_lat=lat;
    input(i).g_long=lon;
